...
save_heatmap <- function(ht, pdf = TRUE, png = TRUE, svg = TRUE, prefix, outdir){
    ...
    if(pdf){
        ...
        max1 <- max(rownames_width1, colnames_height1)
        print(max1)
        pdf(file1, width = size_pdf[1] + 2*max1, height = size_pdf[2] + 2*max1 , bg="white")
    ...
    if(svg){
        size_pdf <- calc_ht_size(ht)
        file1 <- paste0(outdir,'/', prefix, '.heatmap.svg')
        #rownames_width1 <- convertX(ht@row_names_param[["max_width"]], "inches", valueOnly = TRUE)
        #colnames_height1 <- convertY(ht@column_names_param[["max_height"]], "inches", valueOnly = TRUE)
        #max1 <- max(rownames_width1, colnames_height1)
        #print(max1)
        ...
        max1 <- max(rownames_width1, colnames_height1)
        print(max1)
        svglite(file1, width = size_pdf[1] + 2*max1, height = size_pdf[2] + 2*max1 , bg="white")
        draw(ht, merge_legend = TRUE)
        dev.off()
    }
    if(png){
        ...
        max2 <- max(rownames_width2, colnames_height2)
        print(max2)
        png(file2, width = size_png[1]+2*max2, height = size_png[2]+2*max2, bg="white", units = 'cm', res = 300)
...
##pheatmap保存图片
save_pheatmap <- function(ht, pdf = TRUE, png = TRUE, svg = TRUE, prefix, outdir){
    library(svglite)
    if(!dir.exists(outdir)){
        dir.create(outdir, recursive=T)
    }
    ...
    if(svg){
        ...
        height <- convertY(unit(sum(ht[["gtable"]][["heights"]])), "inches", valueOnly = TRUE) + 0.5
        file1 <- paste0(outdir,'/', prefix, '.heatmap.svg')
        svglite(file1, width = width ,height = height, bg="white")
        print(ht)
        dev.off()
    }
    if(png){
...
draw_ComplexHeatmap <- function(matrix, scale_or_not = TRUE, scale_by = "columns", center = TRUE, scale = TRUE, heat_col = NULL,
                         ...
                         rowAnnotation = NULL, colAnnotation = NULL, rowAnnotation_col = NULL, colAnnotation_col = NULL, right_annotation = NULL,
                         ...
                         display_sig = FALSE, sig = NULL
                            ...
                            if(!is.null(rowAnnotation)){
                                ...
                                row_annotation <- rowAnnotation(df = rowAnnotation, col = rowAnnotation_col, gp = gpar(col = rowAnnotation_inside_color), border = rowAnnotation_outside_border,
                                                                ...
                                                                annotation_legend_param = list(title_gp = gpar(fontsize = rowAnnotation_legend_title_fontsize, fontface = rowAnnotation_legend_title_fontface), ncol = 1,
                            ...
                            if(!is.null(colAnnotation)){
                                ...
                                col_annotation <- HeatmapAnnotation(df = colAnnotation, col = colAnnotation_col, gp = gpar(col = colAnnotation_inside_color), border = colAnnotation_outside_border,
                                                                ...
                                                                annotation_legend_param = list(title_gp = gpar(fontsize = colAnnotation_legend_title_fontsize, fontface = colAnnotation_legend_title_fontface), ncol = 1,
                            ...
                            ht <- ComplexHeatmap::Heatmap(matrix, col = col_fun, na_col = na_col, border = border, border_gp = gpar(col = border_outside_col), rect_gp = gpar(col = border_inside_col),
                                                          ...
                                                          left_annotation = row_annotation, top_annotation = col_annotation, right_annotation = right_annotation,
